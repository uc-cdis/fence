...
def syncer(db_session):
    ...
    projects = [
        {
            'auth_id': 'TCGA-PCAWG',
            'storage_accesses': [
                {'buckets': ['test-bucket'], 'name': 'test-cleversafe'},
            ]
        },
        {
            'auth_id': 'phs000178',
            'name': 'TCGA',
            'storage_accesses': [
                {'buckets': ['test-bucket2'], 'name': 'test-cleversafe'},
            ],
        },
        {
            'auth_id': 'phs000179',
            'name': 'BLAH',
            'storage_accesses': [
                {'buckets': ['test-bucket3'], 'name': 'test-cleversafe'},
            ],
        },
    ]
    project_mapping = {
    ...
    syncer_obj = DbGapSyncer(
        dbGaP={}, DB=DB, db_session=db_session,
    ...
    for project in projects:
        existing_project = (
            db_session
            .query(Project)
            .filter_by(auth_id=project['auth_id'])
            .first()
        )
