...
def test_sync(
    ...
    if parse_consent_code_config:
        ...
        assert user.project_access == {
            "phs000178.c1": ["read-storage", "read"],
            "phs000178.c2": ["read-storage", "read"],
            "phs000178.c999": ["read-storage", "read"],
            "phs000179.c1": ["read-storage", "read"],
        }
        ...
        user = models.query_for_user(session=db_session, username="USERC")
        assert user.project_access == {
            "phs000178": ["read-storage", "read"],
            "TCGA-PCAWG": ["read-storage", "read"],
            "phs000179": ["read-storage", "read"],
        }
...
def test_dbgap_consent_codes(
    ...
    if parse_consent_code_config:
        ...
        assert user.project_access == {"phs000178": ["read-storage", "read"]}
    ...
    user = models.query_for_user(session=db_session, username="TESTUSERD")
    if parse_consent_code_config:
        assert user.project_access == {"phs000179.c1": ["read-storage", "read"]}
    else:
        assert user.project_access == {"phs000179": ["read-storage", "read"]}
...
def test_sync_from_files(syncer, db_session, storage_client):
    ...
    phsids = {
        "userA": {
            "phs000178": {"read-storage", "read"},
            "phs000179": {"read-storage", "read", "write-storage"},
        },
        "userB": {"phs000179": {"read-storage", "read", "write-storage"}},
    }
    userinfo = {
...
def test_sync_revoke(syncer, db_session, storage_client):
    ...
    phsids2 = {"userA": {"phs000179": {"read-storage", "read", "write-storage"}}}
...
def test_sync_two_phsids_dict(syncer, db_session, storage_client):
    ...
    phsids2 = {"userA": {"phs000180": {"read-storage", "read", "write-storage"}}}

    syncer.sync_two_phsids_dict(phsids1, phsids2)

    assert phsids2 == {
        "userB": {"phs000179": set(["read-storage", "read", "write-storage"])},
        "userA": {
            "phs000178": set(["read-storage", "read"]),
            "phs000179": set(["read-storage", "read", "write-storage"]),
            "phs000180": set(["write-storage", "read-storage", "read"]),
        },
    }
...
def test_sync_two_phsids_dict_override(syncer, db_session, storage_client):
    ...
    phsids2 = {"userA": {"phs000179": {"read-storage", "read"}}}
