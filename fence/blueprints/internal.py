...
POD_USERNAME_LABEL = 'gen3username'
JUPYTER_POD_LABEL = 'hub.jupyter.org/username'
...
def get_username_from_ip(ip):
    ...
    for pod in ret.items:
        if pod.metadata.annotations and POD_USERNAME_LABEL in pod.metadata.annotations:
            return pod.metadata.annotations[POD_USERNAME_LABEL]
        elif pod.metadata.annotations and JUPYTER_POD_LABEL in pod.metadata.annotations:
            return pod.metadata.annotations[JUPYTER_POD_LABEL]
