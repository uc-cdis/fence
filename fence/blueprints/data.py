...
def get_index_document(file_id):
    indexd_server = (
            ...
            flask.current_app.config['HOSTNAME'] + '/index')
    url = indexd_server + '/index/'
    try:
        res = requests.get(url + file_id)
    except Exception as e:
        flask.current_app.logger.error("fail to reach indexd at {0}: {1}".format(url + file_id, e))
    ...
    if res.status_code == 200:
        try:
            ...
            if 'urls' not in json_response or 'metadata' not in json_response:
                flask.current_app.logger.error("urls and metadata are not included in indexd's response {0}".format(url + file_id))
                raise InternalError("Urls and metadata not found")
            return res.json()
        except Exception as e:
            flask.current_app.logger.error("indexd return a response without JSON field {0}: {1}".format(url + file_id, e))
            raise InternalError("Internal error from indexd")
    elif res.status_code == 404:
        flask.current_app.logger.error("indexd can't find {0}: {1}".format(url + file_id, res.text))
...
def resolve_url(url, location, expires, action):
    ...
    if protocol == 's3':
        if 'AWS_CREDENTIALS' in flask.current_app.config and len(flask.current_app.config['AWS_CREDENTIALS']) > 0:
            if location.netloc not in flask.current_app.config['S3_BUCKETS'].keys():
                raise Unauthorized("We don't have permission on this bucket")
            if location.netloc in flask.current_app.config['S3_BUCKETS'].keys() and \
                    flask.current_app.config['S3_BUCKETS'][location.netloc] not in flask.current_app.config['AWS_CREDENTIALS']:
                raise Unauthorized("We don't have credential on this bucket")
        credential_key = flask.current_app.config['S3_BUCKETS'][location.netloc]
        url = flask.current_app.boto.presigned_url(
            location.netloc,
            location.path.strip('/'),
            expires,
            flask.current_app.config['AWS_CREDENTIALS'][credential_key],
