...
def get_all_registered_service_accounts(db=None):
    ...
    for account in current_session.query(ServiceAccountToGoogleBucketAccessGroup).all():
        registered_service_accounts.extend(
            get_registered_service_accounts(account.service_account.google_project_id)
        )
